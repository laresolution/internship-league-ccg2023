  ...
  <v-container class="pa-0" fluid>
    <v-row no-gutters>
      <v-card class="mx-auto pa-8" max-width="80%" outlined>
        <v-card-text class="ms-4 text-h2 text-center font-weight-bold">
          {{ pageTitle }}
        </v-card-text>
        <v-card-text>
    ...
    <v-row no-gutters class="px-16 py-8">
      <v-img id="banner" :alt="altBanner" src="./img/hire-our-interns.jpg">
      </v-img>
    </v-row>
    <v-row no-gutters justify="center">
...
export default {
  ...
  data() {
    return {
      ...
      invitationText:
        'Be confident in the person you wish to collaborate with to have the required skills, resources and support to contribute to the success of your enterprise. Use this button to browse our collaborators portfolio!',
      textBtnRedirectToMembers: 'Discover your potential hire!',
    }
  },
  computed: {
    ...mapGetters('customers', ['customers', 'currentCustomer']),
    regularComputedProperty() {
      return true
    },
  },
  methods: {
    ...mapActions('customers', ['loadCustomers', 'addCustomer']),
